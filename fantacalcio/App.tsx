...
function App() {
  ...
  useEffect(() => {
    ...
    const userName = urlParams.get('name'); // Get the user's name

    if (userIdToLogin) {
      // Login via link is always a guest session.
      // Pass the user's name so they can be created on-the-fly if they don't exist in the state.
      actions.login(userIdToLogin, true, userName || undefined);
      
      // Clean up URL to avoid re-login on refresh
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  }, [actions]);

  if (!loggedInUserId) {
    // If there's a guest session ID, it means a guest user has logged out.
    // Show them their personal re-login page.
    if (guestSessionId) {
        return <UserReLoginPage />;
    }
    // Otherwise, show the main admin login screen.
    return <LoginScreen />;
  }
