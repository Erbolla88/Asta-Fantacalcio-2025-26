...
export const AdminPanel: React.FC<{currentUser: User}> = ({currentUser}) => {
    ...
    const handleInitialize = () => {
        ...
        actions.initializeAuction(initialCredits).catch(err => {
            setError(err instanceof Error ? err.message : t('errorGeneric'));
        });
    };
    ...
    const handleAddPlayer = (e: React.FormEvent) => {
        ...
        setError(null);
        actions.addPlayerManually(newPlayer).then(() => {
    ...
    const handleClipUpload = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!clipFile) {
            setError(t('errorClipForm'));
            return;
        }
        setError(null);
        setClipSuccess(null);
        ...
        try {
            ...
            await actions.setSpecialUserClipUrl(dataUrl);
            setClipSuccess(t('adminClipSuccess'));
            setClipFile(null);
            (e.target as HTMLFormElement).reset();
    ...
    return (
        <div className="bg-brand-surface p-6 rounded-lg shadow-lg w-full h-full">
            ...
            {clipSuccess && <div className="bg-green-900 border border-green-700 text-green-100 px-4 py-3 rounded-md mb-4">{clipSuccess}</div>}
            ...
            <div className="space-y-6">
                            ...
                            return (
                                <div key={user.uid} className="flex items-center justify-between bg-slate-800 p-2 rounded-md">
                                    <div>
                                        ...
                                        {user.email && <span className="text-xs text-brand-subtle ml-2 block sm:inline"> - {user.email}</span>}
                                    </div>
                ...
                <div className="p-4 border border-slate-700 rounded-lg">
                 ...
                 <div className="p-4 border border-slate-700 rounded-lg">
                    <h3 className="font-semibold text-lg mb-2">{t('adminSectionSpecialClip')}</h3>
                     {status !== 'SETUP' ? (
                        <p className="text-brand-subtle text-sm">{t('adminCustomLogosDisabled')}</p>
                    ) : (
                        <form onSubmit={handleClipUpload} className="space-y-3">
                            <p className="text-sm text-brand-subtle">{t('adminSpecialClipInstruction')}</p>
                            <label className="w-full block px-4 py-2 text-center rounded-md font-semibold bg-slate-600 text-white hover:bg-slate-500 cursor-pointer">
                                {clipFile ? clipFile.name : t('adminChooseMp3')}
                                <input type="file" accept="audio/mpeg" onChange={e => setClipFile(e.target.files?.[0] || null)} className="hidden" />
                            </label>
                            <Button type="submit" disabled={!clipFile || isLoading} className="w-full">
                                {isLoading ? t('loading') : t('adminUploadClip')}
                            </Button>
                        </form>
                    )}
                </div>
                ...
                <div className="p-4 border border-slate-700 rounded-lg">
                 ...
                 <div className="p-4 border border-amber-600 rounded-lg bg-amber-900 bg-opacity-20">
                    ...
                    <Button 
                        onClick={() => actions.startTestAuction().catch(e => setError(e instanceof Error ? e.message : t('errorGeneric')))} 
                ...
                {status === 'READY' && (
                    <div className="p-4 border border-slate-700 rounded-lg bg-slate-900">
                        ...
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <Button onClick={() => actions.startAuction().catch(e => setError(e instanceof Error ? e.message : t('errorGeneric')))} disabled={!allUsersReady} variant="primary">
