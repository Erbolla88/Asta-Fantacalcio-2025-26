...
export const AuctionQueue: React.FC = () => {
    const { players, auctionQueue, currentPlayerIndex, status } = useAuctionStore();

    // Do not show the queue before the auction starts or after it ends
    ...
    return (
        <div className="mt-8 bg-brand-surface p-4 rounded-lg shadow-lg w-full">
            <h3 className="text-xl font-bold mb-4">Prossimi Giocatori in Asta</h3>
            {upcomingPlayerIndices.length > 0 ? (
                ...
                <p className="text-brand-subtle">Nessun altro giocatore in coda.</p>
            )}
        </div>
    );
};