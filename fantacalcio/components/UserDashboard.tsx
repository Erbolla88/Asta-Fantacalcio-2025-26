...
export const UserDashboard: React.FC<{ user: User }> = ({ user }) => {
    ...
    const handleTeamNameBlur = () => {
        if (currentTeamName.trim() && currentTeamName !== user.teamName) {
            actions.setTeamName(user.uid, currentTeamName.trim()).catch(err => console.error("Failed to set team name:", err));
        }
    };
    ...
    const handleProfilePicChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            try {
                ...
                await actions.setProfilePicture(user.uid, dataUrl);
            } catch (error) {
    ...
    return (
        <aside className="bg-brand-surface p-4 rounded-lg shadow-lg w-full h-full flex flex-col">
            ...
            {status === 'READY' && !user.isReady && (
                <div className="my-4">
                    <Button onClick={() => actions.setUserReady(user.uid).catch(e => console.error("Failed to set user ready status:", e))} variant="primary" className="w-full">
                        {t('readyButton')}
                    </Button>
                </div>
            )}
