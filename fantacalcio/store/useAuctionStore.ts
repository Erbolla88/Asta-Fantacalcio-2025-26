...
const initialUsers = [
    ...
    { id: 'user-zup', name: 'Zuppetta'},
    { id: 'user-mat', name: 'Mattia'},
    { id: 'user-gna', name: 'Gnani'},
    { id: 'user-roc', name: 'Roccia'},
    { id: 'user-trs', name: 'Treky & Seghetta'},
    { id: 'user-pri', name: 'Principe'},
    { id: 'user-gin', name: 'Gino'},
    { id: 'user-dan', name: 'Daniel'},
    { id: 'user-ric', name: 'Riccardo'},
];
        ...
        if (get().users.has(userId)) {
            try {
              ...
              if (isGuest) {
                  ...
                  set({ loggedInUserId: userId, guestSessionId: userId });
              } else {
                  // Admin is logging in, clear any old guest session
                  sessionStorage.removeItem('fantacalcio_guestSessionId');
                  set({ loggedInUserId: userId, guestSessionId: null });
              }
            } catch (e) {
              console.warn('Failed to access sessionStorage. Login will not persist.');
              set({ loggedInUserId: userId, guestSessionId: isGuest ? userId : null });
            }
        } else {
