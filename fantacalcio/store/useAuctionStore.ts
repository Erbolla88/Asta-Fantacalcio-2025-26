...
const useAuctionStore = create<AuctionState>((set, get) => ({
  ...
  actions: {
    ...
    login: (userId, isGuest, userName) => {
        ...
        // If user doesn't exist but we have info to create them, do it now.
        // This allows users to join from a link without needing a shared state.
        if (!users.has(userId) && userName) {
            const newUser: User = {
                id: userId,
                name: userName,
                credits: 500, // This will be reset by initializeAuction anyway
                squad: [],
                isReady: false,
