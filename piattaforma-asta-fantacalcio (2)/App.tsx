...
const App: React.FC = () => {
  ...
  // Auto-login effect for users with a specific link
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const userId = urlParams.get('userId');
    if (userId) {
      const userToLogin = users.find(u => u.id === parseInt(userId, 10));
      if (userToLogin) {
  ...
  useEffect(() => {
    ...
    // This effect should only trigger when the current player is marked as sold
    if (!currentPlayer || currentPlayer.status !== ItemStatus.Sold) {
        return;
    }

    // Prevent the effect from re-triggering for the same sold player while waiting
    if (soldAndWaiting === currentPlayer.id) {
        return;
    }
    ...
    const timer = setTimeout(() => {
        // Find the first player in the list that is still on auction
        const nextPlayer = players.find(p => p.status === ItemStatus.OnAuction);

        setCurrentPlayerId(nextPlayer ? nextPlayer.id : null);
        setSoldAndWaiting(null); // Reset for the next sale
    }, 3000); // 3-second delay to show the "SOLD" message
  ...
  const handleLogout = () => {
    setCurrentUser(null);
    // Redirect to the main page without user id
  ...
  if (!currentUser) {
    const loginUsers = users.filter(u => u.role === Role.Admin);
    return <LoginScreen users={loginUsers} onLogin={handleLogin} />;
  }
